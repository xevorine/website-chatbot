...
// --- TAMBAHAN KODE MULAI SINI ---
if (!$group) {
    // Jika data tidak ditemukan di tabel groups
    // Kita coba cek apakah perlu disinkronisasi ulang atau memang ID salah
    echo "<div style='padding: 20px; font-family: sans-serif;'>";
    echo "<h1>❌ Error: Data tidak ditemukan</h1>";
    echo "<p>Group dengan ID: <strong>" . htmlspecialchars($group_id) . "</strong> belum terdaftar di tabel `groups`.</p>";
    echo "<p>Saran: <a href='create_groups_table.php'>Klik di sini untuk Sinkronisasi Data Group</a></p>";
    echo "<a href='groups.php'>Kembali</a>";
    echo "</div>";
    exit; // Stop script di sini agar tidak error ke bawah
}
...
<body class="m-0 p-0 box-border font-sans flex h-screen bg-gray-100">
        ...
        <nav>
            <ul class="list-none m-0 p-0">
                <li class="m-0"><a href="index.php"
                        ...
                        Daftar Warning</a></li>
                <li class="m-0"><a href="group.php"
                        class="block p-4 text-white no-underline transition-all duration-300 border-l-4 border-transparent hover:bg-slate-700 hover:border-l-blue-400 bg-blue-600 border-l-blue-400">📁
                        Groups</a></li>
                <li class="m-0"><a href="#"
                        class="block p-4 text-white no-underline transition-all duration-300 border-l-4 border-transparent hover:bg-slate-700 hover:border-l-blue-400">👥
    ...
    <div class="flex-1 p-8 overflow-y-auto">
        <a href="group.php"
            class="inline-block mb-5 px-4 py-2 bg-slate-700 text-white rounded no-underline transition-all duration-300 hover:bg-slate-600">←
            Kembali ke Groups</a>
        ...
        <div
            ...
            <p class="text-white/90 text-sm relative z-10 mt-2">Kelola informasi grup:
        ...
        <div class="bg-white p-8 rounded-lg shadow">
            ...
            <div class="grid grid-cols-2 gap-4 mb-8">
                <div class="bg-gray-50 p-4 rounded border-l-4 border-indigo-500">
                    ...
                    <div class="text-2xl font-bold text-slate-700 mt-1"><?php echo $warn_stats['total_warnings']; ?>
                    </div>
                </div>
                <div class="bg-gray-50 p-4 rounded border-l-4 border-indigo-500">
            ...
            <form method="POST" action="update_group.php">
                ...
                <div class="mb-5">
                    <label for="group_id_display" class="block mb-2 font-bold text-slate-700">Group ID
                        (Read-only)</label>
                    <input type="text" id="group_id_display" value="<?php echo htmlspecialchars($group_id); ?>" disabled
                        class="w-full p-2 border border-gray-300 rounded text-sm font-sans">
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="group_name" name="group_name"
                        ...
                        class="w-full p-2 border border-gray-300 rounded text-sm font-sans focus:outline-none focus:border-indigo-500 focus:shadow-md focus:shadow-indigo-300/30">
                </div>

                <div class="mb-5">
                    <label for="description" class="block mb-2 font-bold text-slate-700">Deskripsi</label>
                    <textarea id="description" name="description" placeholder="Masukkan deskripsi group (opsional)"
                        class="w-full p-2 border border-gray-300 rounded text-sm font-sans resize-vertical min-h-24 focus:outline-none focus:border-indigo-500 focus:shadow-md focus:shadow-indigo-300/30"><?php echo htmlspecialchars($group['description'] ?? ''); ?></textarea>
                </div>

                <div class="flex gap-2 mt-8">
                    <button type="submit"
                        class="flex-1 px-5 py-2 bg-green-500 text-white rounded font-bold transition-all duration-300 hover:bg-green-600">💾
                        Simpan</button>
                    <a href="group.php"
                        class="flex-1 px-5 py-2 bg-red-500 text-white rounded font-bold transition-all duration-300 hover:bg-red-600 no-underline text-center">❌
                        Batal</a>
                </div>
            </form>
        </div>
